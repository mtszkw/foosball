...
{
    const string title = mode == detection::Mode::BLUE_PLAYERS ? "Blue players detection frame"
                                                               : "Red players detection frame";
...
void detection::trackBall(bool trackingEnabled, bool debugMode, FoundBallsState &foundBallsState,
                          double deltaTicks, int &founded, int &counter, cv::Mat &frame,
                          cv::Mat &nextFrame, cv::Mat &restul)
{
...
cv::Scalar detection::getColorForMode(detection::Mode mode, int colorIndex)
{
    if (mode == detection::Mode::BALL)
    {
        if (colorIndex == 0)
            ...
            return cv::Scalar(29, 255, 203);
    }
    else if (mode == detection::Mode::BLUE_PLAYERS)
    {
        if (colorIndex == 0)
            return cv::Scalar(90, 50, 50);
        else
            return cv::Scalar(130, 255, 255);
    }
    else
    {
        if (colorIndex == 0)
...
cv::Mat detection::getMaskForMode(Mode mode, cv::Size size)
{
    ...
    if (mode != Mode::BALL)
    {
        ...
        mask(cv::Range(41 * mask.rows / 60, mask.rows),
             cv::Range(9 * mask.cols / 240, 33 * mask.cols / 240)) = 0;
...
{
    cv::findContours(rangeRes, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
...
void detection::FoundBallsState::setCenter(cv::Point x) { center = x; }
...
{
    cv::findContours(rangeRes, players, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
